...
export const initializeProducts = (products: IProduct[], length: number): AppActions => ({
...
export const requestPage = (products: IProduct[], page: number): AppActions => ({
    ...
    return (dispatch: Dispatch<AppActions> | any, getState: () => AppState) => {
        ...
        addNewProduct(paylod)
            .then((res) => {
                ...
                dispatch(initializeProducts([...OLD_PRODUCTS, product], OLD_PRODUCTS?.length + 1))
                dispatch(unloadState())
            })
            .catch(handleError)
    }
